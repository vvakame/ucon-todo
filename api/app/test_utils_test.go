...
func makeTestDataTodo(t *testing.T, g *goon.Goon, usr *user.User, todo *Todo) *Todo {
	if todo == nil {
		todo = &Todo{}
	}
	if todo.ParentKey != nil && usr == nil {
		usr = &user.User{ID: todo.ParentKey.StringID()}
	} else if usr == nil {
		usr = &user.User{ID: "foobartodo"}
	}
